...
export default function Login() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                // حفظ الدور والاسم في المتصفح
                localStorage.setItem('userRole', data.role); // 'admin' أو 'stagiaire'
                localStorage.setItem('user', data.user);
                
                toast.success(`Accès réussi en tant que ${data.role} !`, { id: load });
        ...
        <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-6 relative overflow-hidden font-sans">
            ...
            <div className="w-full max-w-md relative z-10 text-black">
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-400 font-bold text-[9px] uppercase tracking-[0.3em] mt-2">Portail d'accès ISTA</p>
                </div>

                <div className="bg-white/70 backdrop-blur-xl p-10 rounded-[3rem] shadow-2xl border border-white">
                    <form onSubmit={handleLogin} className="space-y-6">
                        <div>
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2 ml-4 font-bold">Identifiant</label>
