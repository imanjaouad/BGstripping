...
export default function Navbar() {
    ...
    // تحديث الساعة كل ثانية لإضفاء حيوية على النظام
    useEffect(() => {
        const timer = setInterval(() => setTime(new Date()), 1000);
        return () => clearInterval(timer);
    }, []);

    // دالة التحكم في شكل الروابط (Active / Inactive)
            ...
            <div className="max-w-full mx-auto">
                <div className="flex justify-between items-center h-20">
                    ...
                    <div className="hidden lg:flex items-center gap-1">
                        ...
                        {/* --- الـرابـط الـجديـد (SALLES) --- */}
                        <Link to="/salles" className={getLinkClass("/salles")}>🏫 Salles</Link>

                        <Link to="/profile" className={getLinkClass("/profile")}>⚙️ Profile</Link>
                    </div>
                    ...
                    <div className="flex items-center gap-6 border-l border-slate-100 pl-6">
                        ...
                        <div className="flex items-center gap-3 bg-slate-50 p-1.5 pr-4 rounded-2xl border border-slate-100 shadow-sm">
                            <div className="w-8 h-8 bg-slate-800 rounded-xl flex items-center justify-center text-white font-black text-xs">
                                K
                            </div>
                            <div className="hidden md:block text-left">
                                <div className="text-[9px] font-black text-slate-400 uppercase leading-none italic">Verified Admin</div>
                                <div className="text-xs font-bold text-slate-800 tracking-tight">Kamal Dev</div>
                            </div>
                        </div>
                    </div>
