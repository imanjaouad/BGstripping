...
export default function EmploiGrid() {
    ...
    const confirmPlan = async (idAff, idSalle) => {
        ...
        if (res.ok) {
            toast.success("Enregistré dans la base SQL ✅");
            setSelection(null);
            fetchData();
        }
    };

    if (loading) return <div className="p-40 text-center font-black animate-pulse text-[#006064] uppercase tracking-[0.5em]">Reconstruction de la grille...</div>;
        ...
        <div className="p-6 pt-32 bg-[#f8fafc] min-h-screen font-sans text-black">
            <div className="max-w-full mx-auto space-y-8">
                ...
                <div className="bg-white border-b-4 border-black p-8 shadow-xl rounded-[2rem] flex flex-col lg:flex-row justify-between items-center gap-6">
                    <div>
                        <h1 className="text-3xl font-black uppercase italic tracking-tighter text-[#006064]">Session {moisActuel} {anneeActuelle}</h1>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-widest tracking-[0.3em] mt-1">Management Hebdomadaire</p>
                    </div>

                    <div className="flex bg-gray-200 p-1 rounded-[1.5rem] border-2 border-black">
                        {[1, 2, 3, 4].map(num => (
                    ...
                    <div className="bg-gray-50 p-2 rounded-2xl border-2 border-black">
                        <select className="bg-transparent font-black text-xs outline-none uppercase" value={selectedGroup} onChange={(e) => setSelectedGroup(e.target.value)}>
                            {[...new Set(affectations.map(a => a.groupe?.code_groupe))].filter(Boolean).map(g => <option key={g} value={g}>Groupe: {g}</option>)}
                        </select>
                    </div>
                </div>

                {/* --- THE GRID --- */}
                <div className="bg-white p-2 border-2 border-black shadow-2xl rounded-[2.5rem] overflow-hidden">
                                                    ...
                                                    return (
                                                        <td key={h} colSpan={span} className={`border-2 border-black ${style.bg}`}>
                                                            <div className="flex flex-col items-center justify-center text-center h-full p-2">
                                                                <div className="font-black text-xs text-black border-b border-black/10 w-full mb-1 uppercase leading-none pb-1 tracking-tighter">{seance.affectation?.module?.code_module}</div>
                ...
                {/* --- HOLIDAYS BANNER --- */}
                {holidays.length > 0 && (
                    <div className="bg-white border-2 border-black p-6 shadow-[10px_10px_0px_rgba(0,0,0,1)] rounded-2xl">
                        <h3 className="text-xs font-black uppercase tracking-[0.4em] text-[#006064] mb-4 border-l-8 border-[#006064] pl-4">Événements {moisActuel}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            {holidays.map((v, i) => (
                                <div key={i} className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex items-center gap-4">
                                    <span className="bg-[#00acc1] text-white p-2 rounded-xl font-black text-xs">{v.jours}</span>
                ...
                {selection && selection.end && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-[2000] p-4 text-black font-bold">
                        <div className="w-full max-w-3xl bg-white border-4 border-black shadow-[30px_30px_0px_rgba(0,0,0,1)] animate-in zoom-in duration-200">
                            ...
                            <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div>
                                    <h3 className="text-[10px] font-black text-gray-400 uppercase mb-4 tracking-[0.2em] border-b pb-2 text-center">1. Module Selection</h3>
                                    <div className="space-y-1.5 max-h-80 overflow-y-auto pr-2 custom-scroll">
                                ...
                                <div>
                                    <h3 className="text-[10px] font-black text-gray-400 uppercase mb-4 tracking-[0.2em] border-b pb-2 text-center">2. Room Allocation</h3>
                                    <div className="grid grid-cols-2 gap-1.5 max-h-80 overflow-y-auto pr-2 custom-scroll">
                                        {options.availableSalles.map(s => (
                                            <button key={s.id_salle} disabled={!selection.id_aff} onClick={() => confirmPlan(selection.id_aff, s.id_salle)}
                                                className={`p-4 border-2 font-black text-[9px] transition-all ${selection.id_aff ? 'bg-white border-gray-200 hover:bg-[#00acc1] hover:text-white' : 'opacity-20 cursor-not-allowed'}`}>
                                                📍 {s.nom_local}
                                            </button>
                                        ))}
                                    </div>
