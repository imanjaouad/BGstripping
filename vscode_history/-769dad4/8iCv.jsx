...
export default function EmploiGrid() {
    ...
    if (loading) return <div className="p-40 text-center font-black animate-pulse">BOOTING...</div>;
    ...
    return (
        <div className="p-4 pt-32 bg-[#f8fafc] min-h-screen font-sans text-black">
            <div className="max-w-full mx-auto space-y-6">
                        ...
                        <div className="flex bg-gray-100 p-1 rounded-2xl border-2 border-black">
                            {[1, 2, 3, 4].map(num => (
                                <button key={num} onClick={() => setSelectedWeek(num)}
                                    className={`px-5 py-2 rounded-xl text-xs font-black transition-all ${selectedWeek === num ? 'bg-black text-white' : 'text-gray-400'}`}>
                                    S{num}
                                </button>
                            ))}
                        </div>
                ...
                <div id="printable-area" className="bg-white p-4 border-2 border-black shadow-2xl rounded-sm">
                        ...
                        <table className="w-full border-collapse border-2 border-black table-fixed">
                            <thead>
                                <tr className="bg-black text-white font-black text-[9px] print:text-black">
                                    <th className="p-2 w-20 border-2 border-black bg-[#006064] print:bg-gray-100">Jour</th>
                    ...
                    {/* Official Footer */}
                    <div className="hidden print:grid grid-cols-3 gap-6 mt-4 text-center">
                        <div className="border-t border-black pt-1 text-[8px] font-black uppercase">{settings?.pied_groupe_gauche || 'Formateur'}</div>
                        <div className="border-t border-black pt-1 text-[8px] font-black uppercase">{settings?.pied_groupe_milieu || 'Surveillant'}</div>
                        <div className="border-t border-black pt-1 text-[8px] font-black uppercase">{settings?.pied_groupe_droit || 'Directeur'}</div>
                    </div>
                </div>
            </div>

            {/* --- 🖨️ نظام الضغط العالي والتحكم في العرض --- */}
            <style>{`
                @media print {
                    /* 1. تصغير الخط والحجم العام */
                    body { background: white !important; font-size: 8pt !important; -webkit-print-color-adjust: exact; }
                    nav, .navbar, .print\\:hidden, button, select { display: none !important; }
                    
                    /* 2. التحكم في العرض (نقصنا العرض هنا) */
                    #printable-area { 
                        width: 92% !important; /* الجدول غايشغل 92% من العرض فقط */
                        margin: 0 auto !important; /* توسيط الجدول في الورقة */
                    ...
                    th, td { border: 0.5pt solid black !important; padding: 1px !important; }
                    
                    /* 4. إعدادات الورقة (A4 Landscape) */
                    @page { size: A4 landscape; margin: 0.8cm !important; }

                    /* 5. تصغير نصوص الحصص */
                    .font-black { font-size: 8pt !important; }
                    .text-[8px], .text-[9px] { font-size: 7pt !important; }
                }
