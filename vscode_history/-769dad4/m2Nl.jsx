...
export default function EmploiGrid() {
    ...
    const getAvailableOptions = () => {
        ...
        // 1. كنعزلو غير الحصص ديال هاد السيمانة والنهار المختار
        const occupiedSeances = emploiData.filter(s => 
        ...
        const occupiedSallesIds = occupiedSeances.map(s => parseInt(s.id_salle));
        const occupiedProfsIds = occupiedSeances.map(s => s.affectation?.id_formateur).filter(id => id !== null);

        // 2. تصفية القاعات المتاحة
        const freeSalles = salles.filter(s => !occupiedSallesIds.includes(parseInt(s.id_salle)));

        // 3. تصفية المواد (حسب القسم المختار + الأستاذ مسالي في هاد السيمانة)
