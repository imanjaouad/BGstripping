...
app.delete("/livres/:id", (req, res) => {
  ...
  if (livres.length === nouveauxLivres.length) {
    return res.status(500).json({ message: "L'id du livre que vous avez tapé est introuvable !" });
  }
  if (writeBooks(res, nouveauxLivres)) {
    res.status(201).json({ message: "CONFIRMATION: Attention tu es en train de supprimer un livre peut-être important pour toi !" });
  }
});
