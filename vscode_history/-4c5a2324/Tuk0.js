...
app.delete("/livres/:id", (req, res) => {
  ...
  if (livres.length === newBooks.length) {
    return res.status(500).json({ message: "L'id du livre que vous avez tapez est introuvable !" });
  }
  if (writeBooks(res, nouveauxLivres)) {
    res.status(201).json({ message: "CONFIRMATION: Attention tu es entrain de supprimer un livre peut etre important pour toi !" });
  }
});
...
app.delete("/livres/:id", (req, res) => {
  ...
  if (writeBooks(res, nouveauxLivres)) {
    res.status(201).json({ message: "Le livre a été supprimé avec succès !",livreSupprime: livreASupprimer
    });
  }
});