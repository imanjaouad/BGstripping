...
app.delete("/livres/:id", (req, res) => {
  const livres = readBooks(res); // lit le fichier JSON
  const id = parseInt(req.params.id);

  // Cherche le livre à supprimer
  ...
  // Crée une nouvelle liste sans le livre supprimé
  const nouveauxLivres = livres.filter((l) => l.id !== id);

  // Sauvegarde la nouvelle liste
  writeBooks(nouveauxLivres, res); // attention à l'ordre des paramètres dans ta fonction writeBooks

  // Renvoie un message de confirmation avec les infos du livre supprimé
  res.status(200).json({
