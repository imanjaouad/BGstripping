...
app.put("/livres/:id", (req, res) => {
  ...
  if (!livre) {
    return res.status(500).json({ message: " confirmation : Le livre est introuvable" });
  }
...
app.delete("/livres/:id", (req, res) => {
  ...
  if (writeBooks(res, nouveauxLivres)) {
    res.status(201).json({ message: "Attention tu es entrain de supprimer un livre peut etre important pour toi !" });
  }
});
