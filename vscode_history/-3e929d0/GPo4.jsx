...
import FormateurView from './components/FormateurView'; // <-- استيراد صفحة الأساتذة

// كومبوننت مطور لحماية المسارات بناءً على الـ Role
const PrivateRoute = ({ children, roleRequired }) => {
    const role = localStorage.getItem('userRole'); // 'admin' أو 'stagiaire' أو 'formateur'
    ...
    if (roleRequired && role !== roleRequired) {
        // توجيه ذكي: إذا حاول المستخدم الدخول لمكان غير مسموح له
        if (role === 'admin') return <Navigate to="/" />;
        if (role === 'formateur') return <Navigate to="/enseignant" />;
        return <Navigate to="/consultation" />;
    }
    ...
    return (
        <BrowserRouter>
            ...
            <Routes>
                ...
                {/* 3. صفحة الأساتذة: محمية (للأساتذة فقط) */}
                <Route path="/enseignant" element={
                    <PrivateRoute roleRequired="formateur">
                        <FormateurView />
                    </PrivateRoute>
                } />

                {/* 4. صفحات الإدارة: محمية (للأدمن فقط) */}
                <Route path="/*" element={
