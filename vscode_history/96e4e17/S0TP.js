...
consumeMessages('notification_queue', ['order.created', 'order.status.updated'], async (payload) => {
    ...
    if (payload.event === 'order.created') {
        msgText = `Votre commande ${data.orderId} a été créée avec succès. Montant: ${data.totalAmount}€`;
    } 
...
const PORT = 3004;
app.listen(PORT, () => console.log(`Notification Service prêt sur le port ${PORT}`));