...
export default function EtudiaModules() {
  ...
  useEffect(() => {
    ...
    // Fonction fetch ultra-flexible + Logs pour debug
    const fetchResource = (url, type) => {
      return fetch(url)
        .then(res => {
        ...
        .then(json => {
          console.log(`Réponse API (${type}) :`, json); // <--- REGARDE LA CONSOLE F12
          ...
          // Cas 3 : { data: [...] } (Laravel resource collection standard)
          if (json.data && Array.isArray(json.data)) return json.data;

          return [];
        })
        .catch(err => {
          console.warn(`Erreur ou vide pour ${type}`, err);
          return [];
        });
    };
    ...
    Promise.all([
      ...
      fetchResource(`${API_BASE}/modules/${selectedModule.id}/efms`, "EFMS"), // <--- C'est celui-là qu'on surveille
      fetchResource(`${API_BASE}/modules/${selectedModule.id}/courses`, "COURS")
    ])
    .then(([ccs, efms, courses]) => {
      setResources({ ccs, efms, courses });
    })
