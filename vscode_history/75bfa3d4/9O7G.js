...
app.post("/login",async (req,res)=>{
    ...
    // vérification de mot de passe en le comparent au mdp hashé stocké ! :
    const isValid = await bcrypt.compare(password,user.password);
    // si le mot de passe est incorrect retourne erreur 404:
    if(!isValid) return res.status(505)json({message : "le mot de passe est incorrect !"});
    // création d'un token JWT contenant l'email de l'utilisateur :
