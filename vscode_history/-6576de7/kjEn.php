...
public function login(Request $request) {
    // كنجيبو الإعدادات اللي حطيتي في صفحة البروفايل من الداتابيز
    $settings = Parametre::find(1);

    if (!$settings) {
        return response()->json(['success' => false, 'message' => 'System error: Settings not found'], 500);
    }

    // 1. التحقق واش هادا أدمـن (Admin)
    if ($settings->login_admin === $request->login && $settings->password_admin === $request->password) {
        return response()->json([
            'success' => true, 
            'role' => 'admin', // هادي ضرورية باش React يعرفو أدمن
            'user' => $settings->login_admin
        ]);
    }

    // 2. التحقق واش هادا مـتـدرّب (Stagiaire)
    // كيمشي يقارن مع الخانات الجداد اللي زدنا في البروفايل
    if ($settings->stagiaire_login === $request->login && $settings->stagiaire_pass === $request->password) {
        return response()->json([
            'success' => true, 
            'role' => 'stagiaire', // هادي كتديه لصفحة العرض فقط
            'user' => 'Espace Stagiaire'
        ]);
    }
