...
public function getAvancement() {
    ...
    try {
        ...
        // حماية: هاد الحقول إجبارية فـ SQL، إلى ماجاوش من React كنعطيوهم 1
        $data['id_filiere'] = $request->id_filiere ?? 1;
        $data['id_efp'] = $request->id_efp ?? 1;
        $data['id_proprietaire'] = $request->id_proprietaire ?? 1;
        $data['statut'] = $request->statut ?? 'actif';
        
        // تحويل سمية الحقل من React (type) لـ SQL (type_formation) إذا كان مختلف
        if(isset($request->type)) {
            $data['type_formation'] = $request->type;
        }

        $g = \App\Models\Groupe::create($data);
        return response()->json(['success' => true, 'data' => $g]);

    } catch (\Exception $e) {
        // هاد الميساج هو اللي غايبان ليك دابا فـ الـ Toast عوض undefined
...
public function login(Request $request) {
    ...
    // 1. التحقق من الأدمن (جدول parametres)
    $settings = \App\Models\Parametre::find(1);
    if ($settings->login_admin === $login && $settings->password_admin === $pass) {
        return response()->json(['success' => true, 'role' => 'admin', 'user' => $login]);
    }

    // 2. التحقق من المتدرب (جدول parametres)
    if ($settings->stagiaire_login === $login && $settings->stagiaire_pass === $pass) {
        return response()->json(['success' => true, 'role' => 'stagiaire', 'user' => 'Stagiaire']);
    }

    // 3. التحقق من الأستاذ (الارتباط بجدول formateurs)
    $formateur = \App\Models\Formateur::where('matricule', $login)
                                     ->where('mdp_defaut', $pass)
                                     ->first();
    ...
    if ($formateur) {
        return response()->json([
            'success' => true, 
            'role' => 'formateur', 
            'user' => $formateur->nom . ' ' . $formateur->prenom,
            'id_formateur' => $formateur->id_formateur // هادا هو الساروت لي كيجيب لومبلوا
        ]);
    }

    return response()->json(['success' => false, 'message' => 'Incorrect'], 401);
}
