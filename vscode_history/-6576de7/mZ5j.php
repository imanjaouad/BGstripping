...
class IstaController extends Controller
{
    ...
    public function deleteSalle($id) {
        ...
        // --- PARTIE KANBAN (DRAG & DROP) ---

    // 12. Hada bach nsjlo Tghyir melli t-glisser l-carte
    public function dropAffectation(Request $request) {
        // 1. N9llbo 3la l'affectation b ID
        $affectation = Affectation::find($request->id_affectation);
        
        if ($affectation) {
            // 2. Nbeddlo ليها l-formateur (Wakha ykoun NULL)
            $affectation->id_formateur = $request->id_formateur;
            $affectation->save();
            
            return response()->json(['success' => true, 'message' => 'Affectation mise à jour']);
        }
        
        return response()->json(['success' => false, 'message' => 'Non trouvé'], 404);
    }
        // 14. Jib Emploi du Temps (Seances)
    public function getEmploi() {
        // Khasna njbo kolchi: Jour, HeureDebut, HeureFin, Groupe, Salle
        $emploi = \App\Models\Seance::with([
            'affectation.groupe', 
            'affectation.module', 
            'affectation.formateur',
            'salle' // Smiyet l'salle
        ])->get();
