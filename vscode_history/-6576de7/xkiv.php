...
public function login(Request $request) {
    ...
    // 1. التحقق من "الأدمـن" (جدول parametres)
    ...
    // 2. التحقق من "المتدرّب" (جدول parametres)
    if ($settings->stagiaire_login === $login && $settings->stagiaire_pass === $pass) {
        return response()->json(['success' => true, 'role' => 'stagiaire', 'user' => 'Espace Stagiaire']);
    }

    // 3. الارتباط الديناميكي: التحقق من "الأستاذ" (جدول formateurs)
    // كنقلبو بالـ Matricule (Login) و mdp_defaut (Password)
    ...
    if ($formateur) {
        return response()->json([
            ...
            'id_formateur' => $formateur->id_formateur // ضروري باش نجبدو لومبلوا ديالو بوحدو
        ]);
    }
