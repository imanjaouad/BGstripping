...
class IstaController extends Controller
{
    ...
    public function deleteSeance($id) {
        ...
        // --- تدبير الأساتذة (CRUD Formateurs) ---

    // 16. إضافة أستاذ جديد
    public function storeFormateur(Request $request) {
        $f = new Formateur();
        $f->matricule = $request->matricule;
        $f->nom = $request->nom;
        $f->prenom = $request->prenom;
        $f->type_contrat = $request->type_contrat;
        $f->mh_statutaire = $request->mh_statutaire;
        $f->nbr_semaines = $request->nbr_semaines;
        $f->metier = $request->metier;
        $f->save();
    ...
    public function storeFormateur(Request $request) {
        ...
        return response()->json(['message' => 'Ajouté', 'data' => $f]);
    }
