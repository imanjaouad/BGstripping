...
function EtudiaLanding() {
  ...
  return (
    <div className="container">
      ...
      {/* --- DÉCORATION (CADRE DE TRIANGLES) --- */}
<div className="decoration-frame">
  {/* HAUT (Pointe vers le bas) */}
  <div className="edge edge-top">
    {[...Array(20)].map((_, i) => <div key={`top-${i}`} className="triangle"></div>)}
  </div>

  {/* BAS (Pointe vers le haut) */}
  <div className="edge edge-bottom">
    {[...Array(20)].map((_, i) => <div key={`bottom-${i}`} className="triangle"></div>)}
  </div>

  {/* GAUCHE (Pointe vers la droite) */}
  <div className="edge edge-left">
    {[...Array(12)].map((_, i) => <div key={`left-${i}`} className="triangle"></div>)}
  </div>

  {/* DROITE (Pointe vers la gauche) */}
  <div className="edge edge-right">
    {[...Array(12)].map((_, i) => <div key={`right-${i}`} className="triangle"></div>)}
  </div>
</div>
    </div> // Fin du return
  );
