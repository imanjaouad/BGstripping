...
<main class="main-content">
    ...
    <div>
        ...
        @php
        $total = $recentActivity->sum(function ($a) {
            $volumeSaute = $a->volume_saute ?? 0;
            $rendement   = $a->rendement ?? 0;
            $hm          = $a->hm_total ?? 0;

            $volumeRestant = $volumeSaute - ($rendement * $hm);

            return max($volumeRestant, 0);
        });
        @endphp

        <div class="kpi-value">{{ number_format($total, 0, ',', ' ') }} m³</div>
    </div>
</div>  

<div class="kpi-value">{{ number_format($total) }} m³</div>
            </div>
        </div>
