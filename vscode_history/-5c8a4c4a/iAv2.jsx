...
{/* Had code kayrsm barre fiha pourcentage dyal Présentiel vs FAD */}
<td className="p-3 w-40 border-r border-gray-200">
    ...
    <div className="flex justify-between text-[10px] mb-1 font-bold uppercase tracking-wider">
        <span className="text-teal-700">{row.mh_pres}h Pres</span>
        <span className="text-orange-600">{row.mh_fad}h Fad</span>
    </div>
    
    {/* L-Barra (Creativity Here) */}
    <div className="w-full bg-gray-200 rounded-full h-2 flex overflow-hidden shadow-inner">
        {/* Partie Khedra (Présentiel) */}
        <div 
            className="bg-teal-500 h-full" 
            style={{ width: `${(row.mh_pres / (parseInt(row.mh_pres) + parseInt(row.mh_fad))) * 100}%` }}
            title="Présentiel"
        ></div>
        {/* Partie Limouniya (FAD) */}
        <div 
