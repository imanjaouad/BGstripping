...
.db-kpi:hover {
  ...
  /* Tableau Operations */
  .db-ops-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:14px; flex-wrap:wrap; gap:10px; }
  .db-ops-count  { font-size:12px; color:#9ca3af; font-style:italic; }
  .db-search-input {
    padding:8px 14px; border:1.5px solid #d1fae5; border-radius:9px;
    font-size:13px; color:#111827; background:#fafffe; outline:none;
    transition:border .18s, box-shadow .18s; width:220px; box-sizing:border-box;
          ...
          <>
            ...
            <div className="db-card" {...anim("0.38s")} style={{ marginBottom:20 }}>
              ...
              {filteredOps.length > 0 ? (
                <FullTable data={filteredOps} showActions={true} />
              ) : (
                <div className="db-empty">
                  {poussages.length === 0 ? "Aucune opération enregistrée." : "Aucun résultat pour la recherche."}
                </div>
              )}
            </div>
            ...
            <div className="db-grid2" style={{ marginBottom:20 }}>
              ...
              <div className="db-card" {...anim("0.42s")}>
                <div className="db-card-header">
                  <div>
                    <p className="db-card-title">Répartition État Machine</p>
                    <p className="db-card-sub">En marche vs En arrêt — toutes opérations</p>
                  </div>
                  <span className="db-pill">{poussages.length} op.</span>
                </div>
                {poussages.length > 0 ? (() => {
              ...
              <div className="db-card" {...anim("0.50s")}>
                <div className="db-card-header">
                  <div>
                    <p className="db-card-title">État Machine par Date</p>
                    <p className="db-card-sub">Opérations En marche / En arrêt par jour</p>
                  </div>
                </div>
                {poussages.length > 0 ? (() => {
                  const byDate = {};
