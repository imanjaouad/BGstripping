...
function Cout() {
  ...
  // ✅ FIX 1 — Redux : correction du chemin (poussages vs list)
  ...
  // ✅ FIX 3 — Sauvegarde automatique à chaque changement
  useEffect(() => {
  ...
  const calculateBudget = () => {
    if (!meterCost || !annualCost || !selectedMonth) return;
  ...
  // ✅ FIX 5 — resetForm ne vide QUE les inputs, pas l'historique
  const resetForm = () => {
    setMeterCost("");
    setAnnualCost("");
    setSelectedMonth("");
  ...
  // ✅ FIX 6 — clearHistory pour vider tout l'historique + localStorage
  const clearHistory = () => {
    setHistory([]);
    localStorage.removeItem("cout-history");
  };
  ...
  useEffect(() => {
    ...
    // ✅ FIX 7 — destroy proprement avant recréation
    ...
    const percents = history.map((h) =>
      totalCost > 0 ? parseFloat(((h.cost / totalCost) * 100).toFixed(1)) : 0
    );
    ...
    // ✅ FIX 8 — cleanup correct du useEffect
    return () => {
      if (chartInstance.current) {
        chartInstance.current.destroy();
        chartInstance.current = null;
  ...
  const inputStyle = {
    ...
    padding: "11px 14px", fontFamily: "'Exo 2', sans-serif",
    fontSize: ".92rem", outline: "none", background: "white",
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{
            ...
            color: "#064E3B", fontSize: "1rem", letterSpacing: 1,
          }}>
            {availableMonths.length > 0
              ? `${availableMonths.length} mois disponible(s) dans les données`
              : "Aucune donnée — enregistrez d'abord des opérations dans Gestion"}
          </div>
        ...
        {availableMonths.length > 0 && (
          <div style={{
            ...
            fontWeight: 700, color: "#16A34A", fontSize: ".9rem",
            whiteSpace: "nowrap",
          }}>
            {poussages.length} enregistrement(s)
          </div>
        )}
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20 }}>
          ...
          <div>
            ...
            <div style={{ position: "relative" }}>
              <input
                type="number" value={meterCost} min="0" step="0.01"
                placeholder="0.00"
          ...
          <div>
            ...
            <div style={{ position: "relative" }}>
              <input
                type="number" value={annualCost} min="0" step="0.01"
                placeholder="0.00"
          ...
          <div>
            <label style={labelStyle}>
              <FaFilter style={{ marginRight: 6, color: "#16A34A" }} />
              Mois
            </label>
            <select
              value={selectedMonth}
      ...
      {isAdmin && history.length > 0 && (
        <div style={{ display: "flex", gap: 20, marginBottom: 28, flexWrap: "wrap" }}>
          ...
          />
          <KpiCard
            label="Coût Moyen / Mois"
            value={fmt(totalCost / history.length)}
            unit=" MAD"
            icon={<FaChartBar />}
            gold
            delay=".2s"
          />
          <KpiCard
            label="Dernier Mois Ajouté"
            value={history[history.length - 1]?.month ?? "—"}
            icon={<FaCalendarAlt />}
            delay=".3s"
          />
        </div>
      ...
      {history.length > 0 && (
        <div className="rendement-banner" style={{ marginBottom: 28 }}>
          ...
          <div style={{ flex: 1 }}>
            <div className="rendement-label">Répartition du Budget Annuel</div>
      ...
      <div className="table-card" style={{ animation: "fadeSlideUp .55s ease both" }}>
        <div className="table-card-title" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span>
            <FaCalendarAlt style={{ color: "#16A34A", marginRight: 8 }} />
          ...
          {history.length > 0 && (
            <span style={{
              ...
              fontFamily: "'Rajdhani',sans-serif",
              fontWeight: 700, color: "#16A34A", fontSize: ".85rem",
            }}>
                ...
                {history.map((h, i) => {
                  ...
                  return (
                    <tr key={i} style={{ animation: `fadeSlideRight .4s ${i * 0.05}s ease both` }}>
                      ...
                      <td>
                        <strong style={{ fontFamily: "'Rajdhani',sans-serif" }}>{h.month}</strong>
                      </td>
                      <td style={{ fontSize: ".82rem", color: "#6B7280" }}>
                        {h.createdAt ?? "—"}
                      </td>
                      <td style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 600 }}>
                        {fmt(h.meterCost)} MAD
                      </td>
                      <td style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 600 }}>
                        {fmt(h.annualCost)} MAD
                      </td>
                      <td style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, color: "#064E3B" }}>
                        {fmt(h.cost)} MAD
                      </td>
                      <td style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, color }}>
                        {pct.toFixed(1)}%
                      </td>
                      <td style={{ minWidth: 140 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <span style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: ".85rem", color, minWidth: 38, textAlign: "right" }}>
                            {pct.toFixed(1)}%
                          </span>
                        </div>
                      </td>
