...
// ✅ Import du contexte d'authentification pour la déconnexion
...
const SIDEBAR_CSS = `
  ...
  .sb-csm-logout-icon {
    width:30px; height:30px; border-radius:8px;
    display:flex; align-items:center; justify-content:center;
    background:rgba(239,68,68,0.1); color:rgba(255,100,100,0.75);
...
function SidebarCasement({ isOpen, toggleSidebar }) {
  ...
  const handleLogout = () => {
    logout();                  // Vide le contexte + storage
  ...
  return (
    <>
      ...
      <div className={isOpen ? "sb-csm-wrapper open" : "sb-csm-wrapper closed"}>
        ...
        {/* ✅ Bouton Déconnexion (remplace "Retour Accueil") */}
